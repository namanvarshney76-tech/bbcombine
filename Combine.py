...
Combines Gmail attachment downloader and Excel GRN processor for both alerts and bbnet2
"""
...
class BigBasketAutomation:
        ...
        try:
            ...
            if "google" in st.secrets and "credentials_json" in st.secrets["google"]:
                ...
                flow = Flow.from_client_config(
                    ...
                    redirect_uri=st.secrets.get("google", {}).get("redirect_uri", "https://bigbasketgrn.streamlit.app/")
                )
    ...
    def process_gmail_workflow(self, config: dict, progress_bar, status_text, log_container):
        ...
        try:
            ...
            base_folder_name = f"Gmail_Attachments_{config['sender_name']}"
    ...
    def _remove_duplicates_from_sheet(self, spreadsheet_id: str, sheet_name: str, duplicate_columns: List[str], log_container):
        ...
        try:
            ...
            # Check if all duplicate columns exist
            missing_columns = [col for col in duplicate_columns if col not in df.columns]
            if missing_columns:
                self._log_message(f"Warning: {missing_columns} columns not found, skipping duplicate removal", log_container)
                return
...
def create_streamlit_ui():
    ...
    # Source selection
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📧 Email Source")
    source = st.sidebar.radio(
        "Select Email Source",
        ["alerts@bigbasket.com", "bbnet2@bigbasket.com"],
        help="Choose which email source to process"
    )
    ...
    with col1:
        ...
        with config_col1:
            days_back = st.number_input(
                ...
                min_value=1,
                max_value=365,
                value=10 if source == "bbnet2@bigbasket.com" else 2,
        ...
        # Show configurations based on source
        with st.expander("📋 Configuration Details", expanded=False):
            if source == "alerts@bigbasket.com":
...
Duplicate Removal: Based on InvoiceNo + SKU Code
    ...
    if source == "alerts@bigbasket.com":
        ...
        excel_config = {
            ...
            'duplicate_columns': ["Item Code", "po_number"]
        }
    else:
        gmail_config = {
            'sender': 'bbnet2@bigbasket.com',
        ...
        excel_config = {
            ...
            'header_row': header_row,
            'duplicate_columns': ["InvoiceNo", "SKU Code"]
        }
    ...
    if workflow_choice == "Gmail to Drive":
        ...
        st.info(f"Downloads Excel attachments from {source} and organizes them in Google Drive")
        ...
        st.info(f"Processes Excel files from Drive and consolidates them into Google Sheets ({excel_config['sheet_name']})")
        ...
        st.info(f"Runs Gmail to Drive first for {source}, then automatically processes Excel files to Sheets")
...
def create_help_section():
    ...
    with st.sidebar.expander("📋 Instructions", expanded=False):
        ...
        2. **Select Email Source** (alerts@bigbasket.com or bbnet2@bigbasket.com)
        ...
        ### Workflow Details:
        - **Gmail to Drive**: Downloads Excel attachments from selected email source
    ...
    with st.sidebar.expander("ℹ️ About", expanded=False):
        st.markdown("""
        **BigBasket Automation v2.0**
        
        This application automates:
        - Gmail attachment downloading from BigBasket (both alerts and bbnet2)
